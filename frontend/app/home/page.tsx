  ...
  return (
    <div onClick={() => router.push(`/watch/${video.id}`)}
      ...
      {video.challenge_state === 'waiting' && (
        <div style={{ position: 'absolute', bottom: 10, left: 10, background: 'linear-gradient(135deg,rgba(126,3,128,0.92),rgb(237,7,15))', color: '#fff', fontSize: '0.62rem', fontWeight: 800, padding: '3px 8px', borderRadius: '20px', letterSpacing: '0.05em' }}>⏳ BIENTÔT</div>
      )}
...
export default function HomePage() {
  ...
  return (
    <>
      ...
      {/* Hero */}
      <div style={{ padding: '0 24px 10px', background: 'radial-gradient(ellipse 80% 60% at 50% -10%,hsl(339, 98%, 49%) 0%,transparent 70%)', textAlign: 'center' }}>
        <h1 style={{ fontFamily: 'Syne,sans-serif', fontWeight: 800, fontSize: 'clamp(0.6rem,3.2vw,1.6rem)', lineHeight: 1.1, marginBottom: 8, whiteSpace: 'nowrap', background: 'linear-gradient(135deg,#f0f0f0,#888)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>
          Podium Challenges{' '}
