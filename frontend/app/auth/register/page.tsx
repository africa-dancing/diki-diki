...
export default function RegisterPage() {
  ...
  async function handleGoogle() {
    setError('');
    if (!firebaseConfigured) { setError('La connexion Google sera bientot disponible.'); return; }
    if (!accepted) { setError('Coche d abord la case des CGU pour continuer avec Google.'); return; }
    setLoading(true);
    try {
      const idToken = await signInWithGoogleIdToken();
      const res = await fetch(`${API}/auth/social`, {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ provider: 'google', token: idToken, accepted: true }),
      });
      const data = await res.json();
      ...
      setError('Connexion Google impossible. Reessaie.'); setLoading(false);
    }
  }
