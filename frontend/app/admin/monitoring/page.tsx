...
function Carte(props: { titre: string; valeur: string; sous?: string }) {
  return (
    <div style={{ background:'#0d0d14', border:'1px solid #1e1e2e', borderRadius:12, padding:'16px 18px' }}>
      <div style={{ fontSize:13, color:'#a0a0c0', fontWeight:600, textTransform:'uppercase', letterSpacing:'0.5px' }}>{props.titre}</div>
      <div style={{ fontFamily:'Syne, sans-serif', fontSize:28, fontWeight:800, color:'#fff', marginTop:6 }}>{props.valeur}</div>
      {props.sous ? <div style={{ fontSize:13, color:'#8a8aa8', marginTop:3 }}>{props.sous}</div> : null}
    </div>
  );
}
  ...
  return (
    <div style={{ display:'flex', minHeight:'100vh', background:'#0a0a0f', color:'#fff', fontFamily:'DM Sans, sans-serif' }}>
      ...
      <div style={{ flex:1, padding:'32px 28px', maxWidth:900 }}>
        ...
        <p style={{ fontSize:15, color:'#a0a0c0', marginBottom:20 }}>Stockage, bande passante et activite en temps reel.</p>

        <button onClick={charger} disabled={loading} style={{ marginBottom:20, padding:'8px 16px', borderRadius:8, border:'1px solid #1e1e2e', background:'#0d0d14', color:OR, fontWeight:600, fontSize:14, cursor:'pointer' }}>
          {loading ? 'Chargement...' : 'Rafraichir'}
        </button>

        {erreur ? <div style={{ color:'#ed070f', marginBottom:16, fontSize:15 }}>{erreur}</div> : null}

        {data ? (
          <>
            <div style={{ background:'#0d0d14', border:'1px solid #1e1e2e', borderRadius:12, padding:'18px 20px', marginBottom:20 }}>
              <div style={{ display:'flex', justifyContent:'space-between', alignItems:'baseline', marginBottom:10 }}>
                <span style={{ fontSize:15, fontWeight:700, color:'#f0f0f0' }}>Bande passante (SUM vues x taille)</span>
                <span style={{ fontSize:15, fontWeight:700, color:couleur }}>{fmt(bwGb)} Go / {QUOTA_GB} Go</span>
              </div>
              ...
              <div style={{ fontSize:13, color:'#8a8aa8', marginTop:6 }}>{pct} % du quota Supabase Free. Alerte orange a {SEUIL_ORANGE_GB} Go, rouge a {SEUIL_ROUGE_GB} Go.</div>
            </div>
            ...
            {/*DKDK_SOLVABILITE_BLOC*/}
                <div style={{ background:'#0d0d14', border:'1px solid #1e1e2e', borderRadius:12, padding:'18px 20px', marginTop:20 }}>
                  <div style={{ fontSize:15, fontWeight:700, color:'#f0f0f0', marginBottom:12 }}>Solvabilite &mdash; ce que la plateforme doit vs sa caisse</div>
                  <div style={{ display:'flex', gap:10, alignItems:'center', marginBottom:14, flexWrap:'wrap' }}>
                    <input type="number" value={caisse} onChange={(e) => setCaisse(e.target.value)} placeholder="Caisse FedaPay (F)"
                      style={{ background:'#0a0a0f', border:'1px solid #1e1e2e', borderRadius:8, padding:'8px 12px', color:'#fff', fontSize:15, width:180 }} />
                    <button onClick={chargerSolvabilite} disabled={solvLoad}
                      style={{ padding:'8px 16px', borderRadius:8, border:'1px solid #1e1e2e', background:'#0d0d14', color:OR, fontWeight:600, fontSize:14, cursor:'pointer' }}>
                      {solvLoad ? 'Calcul...' : 'Calculer'}
                    </button>
                    <span style={{ fontSize:13, color:'#8a8aa8' }}>Montant “Disponible” lu sur le dashboard FedaPay.</span>
                  </div>
                  {solv ? (
                    <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill, minmax(200px, 1fr))', gap:14 }}>
                      ...
                      <div style={{ background:'#0d0d14', border:'1px solid ' + (solv.solvable ? '#1e3a24' : '#3a1e1e'), borderRadius:12, padding:'16px 18px' }}>
                        <div style={{ fontSize:13, color:'#a0a0c0', fontWeight:600, textTransform:'uppercase', letterSpacing:'0.5px' }}>Retirable par admin</div>
                        <div style={{ fontFamily:'Syne, sans-serif', fontSize:28, fontWeight:800, color:(solv.solvable ? '#4ade80' : '#ed070f'), marginTop:6 }}>{fmt(solv.retirable_par_admin)} F</div>
                        <div style={{ fontSize:13, color:(solv.solvable ? '#4ade80' : '#ed070f'), marginTop:3 }}>{solv.solvable ? 'Solvable' : 'INSOLVABLE - ne pas retirer'}</div>
                      </div>
                    </div>
                  ) : <div style={{ fontSize:14, color:'#a0a0c0' }}>Saisis la caisse FedaPay puis clique sur Calculer.</div>}
                </div>

                <div style={{ fontSize:13, color:'#8a8aa8', marginTop:20 }}>Genere le {data?.generated_at}</div>
          </>
        ) : (!loading && !erreur ? <div style={{ color:'#a0a0c0' }}>Aucune donnee.</div> : null)}
      </div>
    </div>
  );
}
