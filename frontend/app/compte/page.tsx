...
// âœ… Ã‰toile rouge â€” identique au logo
const StarRed = () => <span style={{ color: '#FF0000' }}>â˜…</span>;
...
const DISC_EMOJI: Record<string,string> = { danse:'ðŸ’ƒ',chant:'ðŸŽ¤',instrument:'ðŸŽ¸',acapella:'ðŸŽ™ï¸',humour:'ðŸ˜‚',poesie:'ðŸ“œ',conte:'ðŸ“–',musique:'ðŸŽµ' };
const DISC_FR: Record<string,string>    = { danse:'Danse',chant:'Chant',instrument:'Instrument',acapella:'A cappella',humour:'Humour',poesie:'PoÃ©sie',poÃ©sie:'PoÃ©sie',conte:'Conte',musique:'Musique' };
const DISCS       = ['all','danse','chant','humour','poesie','conte','musique'];
const DISC_LABELS: Record<string,string> = { all:'Tous',danse:'ðŸ’ƒ Danse',chant:'ðŸŽ¤ Chant',humour:'ðŸ˜‚ Humour',poesie:'ðŸ“œ PoÃ©sie',conte:'ðŸ“– Conte',musique:'ðŸŽµ Musique' };
const COUNTRIES = [
  {code:'BJ',flag:'ðŸ‡§ðŸ‡¯',name:'BÃ©nin'},{code:'CI',flag:'ðŸ‡¨ðŸ‡®',name:"CÃ´te d'Ivoire"},{code:'SN',flag:'ðŸ‡¸ðŸ‡³',name:'SÃ©nÃ©gal'},{code:'ML',flag:'ðŸ‡²ðŸ‡±',name:'Mali'},
  {code:'BF',flag:'ðŸ‡§ðŸ‡«',name:'Burkina Faso'},{code:'GN',flag:'ðŸ‡¬ðŸ‡³',name:'GuinÃ©e'},{code:'TG',flag:'ðŸ‡¹ðŸ‡¬',name:'Togo'},{code:'CM',flag:'ðŸ‡¨ðŸ‡²',name:'Cameroun'},
  {code:'NG',flag:'ðŸ‡³ðŸ‡¬',name:'Nigeria'},{code:'GH',flag:'ðŸ‡¬ðŸ‡­',name:'Ghana'},{code:'FR',flag:'ðŸ‡«ðŸ‡·',name:'France'},{code:'BE',flag:'ðŸ‡§ðŸ‡ª',name:'Belgique'},
  {code:'CA',flag:'ðŸ‡¨ðŸ‡¦',name:'Canada'},{code:'US',flag:'ðŸ‡ºðŸ‡¸',name:'Ã‰tats-Unis'},
];

// âœ… 'education' ajoutÃ© au TabId
...
const TABS: {id:TabId;emoji:string;label:string}[] = [
  {id:'dashboard',   emoji:'ðŸ“Š', label:'Tableau de bord'},
  {id:'videos',      emoji:'ðŸŽ¬', label:'Mes vidÃ©os'},
  {id:'competitions',emoji:'ðŸ†', label:'CompÃ©titions'},
  {id:'education',   emoji:'ðŸ“š', label:'Ã‰ducation & Savoirs'},
  {id:'finances',    emoji:'ðŸ’³', label:'Finances'},
  {id:'settings',    emoji:'âš™ï¸', label:'ParamÃ¨tres'},
];
...
function StatusBadge({status}:{status:UserVideo['status']}) {
  const cfg = {
    draft:    {bg:'rgba(255,255,255,0.06)',color:'rgba(255,255,255,0.5)',border:'rgba(255,255,255,0.15)',label:'ðŸ“ Brouillon'},
    approved: {bg:'rgba(74,222,128,0.12)', color:'#4ade80',             border:'rgba(74,222,128,0.25)',label:'âœ“ ApprouvÃ©e'},
    pending:  {bg:'rgba(255,170,0,0.1)',   color:'#FFAA00',             border:'rgba(255,170,0,0.25)', label:'â³ En attente'},
    rejected: {bg:'rgba(248,113,113,0.1)', color:'#f87171',             border:'rgba(248,113,113,0.25)',label:'âœ• RejetÃ©e'},
  }[status];
...
function SubmissionModal({video,contests,onClose,onSuccess}:{video:{id:string;title:string;discipline?:string};contests:Contest[];onClose:()=>void;onSuccess:()=>void}) {
  ...
  async function submit(){if(!sel){setErr('Choisis une compÃ©tition.');return;}setLoad(true);setErr('');try{const res=await fetch(`${API}/videos/${video.id}`,{method:'PATCH',headers:{'Content-Type':'application/json',Authorization:`Bearer ${getToken()}`},body:JSON.stringify({status:'pending',contest_id:sel})});const d=await res.json();if(!res.ok)throw new Error(d.message??d.error??'Erreur');setDone(true);setTimeout(()=>{onSuccess();onClose();},2000);}catch(e:any){setErr(e.message);}finally{setLoad(false);}}
  return (
    <div onClick={onClose} style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.78)',backdropFilter:'blur(4px)',display:'flex',alignItems:'center',justifyContent:'center',zIndex:999,padding:16}}>
      <div onClick={e=>e.stopPropagation()} style={{background:'#12121e',border:'1px solid rgba(255,170,0,0.25)',borderRadius:20,width:'100%',maxWidth:460,overflow:'hidden'}}>
        <div style={{background:'linear-gradient(135deg,rgba(255,170,0,0.1),rgba(255,107,0,0.06))',borderBottom:'1px solid rgba(255,170,0,0.15)',padding:'16px 20px',display:'flex',alignItems:'center',justifyContent:'space-between'}}>
          <div><div style={{fontSize:10,color:'#FFAA00',fontWeight:700,letterSpacing:'.1em',marginBottom:3}}>SOUMETTRE POUR VALIDATION</div><div style={{fontSize:16,fontWeight:800,color:'#fff',fontFamily:'Syne, sans-serif'}}>ðŸ† Choisir une compÃ©tition</div></div>
          <button onClick={onClose} style={{width:28,height:28,borderRadius:'50%',background:'rgba(255,255,255,0.06)',border:'1px solid rgba(255,255,255,0.1)',color:'rgba(255,255,255,0.5)',fontSize:14,cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center'}}>âœ•</button>
        </div>
        {done?(<div style={{padding:40,textAlign:'center'}}><div style={{fontSize:44,marginBottom:12}}>ðŸŽ‰</div><p style={{color:'#4ade80',fontWeight:700,fontSize:15,marginBottom:6}}>VidÃ©o soumise pour validation !</p><p style={{color:'rgba(255,255,255,0.4)',fontSize:12,lineHeight:1.6}}>Notre Ã©quipe examinera ta vidÃ©o sous 24 Ã  48h.</p></div>):(
          <div style={{padding:'18px 20px',display:'flex',flexDirection:'column',gap:14}}>
            <div style={{background:'rgba(255,255,255,0.04)',border:'1px solid rgba(255,255,255,0.08)',borderRadius:12,padding:'10px 14px',display:'flex',alignItems:'center',gap:10}}><span style={{fontSize:20}}>{DISC_EMOJI[video.discipline??'']??'ðŸŽ¬'}</span><div><div style={{fontSize:13,fontWeight:600,color:'#fff'}}>{video.title}</div><div style={{fontSize:11,color:'rgba(255,255,255,0.4)'}}>Brouillon â†’ En attente de modÃ©ration</div></div></div>
            <div>
              <label style={{display:'block',fontSize:12,fontWeight:600,color:'rgba(255,255,255,0.5)',marginBottom:8,textTransform:'uppercase',letterSpacing:'.5px'}}>CompÃ©tition * <span style={{color:'rgba(248,113,113,0.7)',fontSize:10,textTransform:'none',fontWeight:400}}>obligatoire</span></label>
              {open.length===0?(<div style={{background:'rgba(248,113,113,0.07)',border:'1px solid rgba(248,113,113,0.18)',borderRadius:12,padding:'12px 14px',fontSize:13,color:'#f87171'}}>âš ï¸ Aucune compÃ©tition ouverte en ce moment.</div>):(
                <div style={{display:'flex',flexDirection:'column',gap:8}}>{open.map(c=>(<div key={c.id} onClick={()=>setSel(c.id)} style={{padding:'12px 14px',borderRadius:12,border:`1px solid ${sel===c.id?'#FFAA00':'rgba(255,255,255,0.1)'}`,background:sel===c.id?'rgba(255,170,0,0.08)':'rgba(255,255,255,0.03)',cursor:'pointer',display:'flex',alignItems:'center',gap:10,transition:'all .2s'}}><div style={{flex:1}}><div style={{fontSize:13,fontWeight:600,color:'#fff',marginBottom:2}}>{c.title}</div><div style={{fontSize:11,color:'rgba(255,255,255,0.4)'}}>{DISC_FR[c.discipline]??c.discipline} Â· {c.status==='active'?'â— En cours':'â—‹ BientÃ´t'}</div></div><div style={{width:20,height:20,borderRadius:'50%',border:`2px solid ${sel===c.id?'#FFAA00':'rgba(255,255,255,0.2)'}`,background:sel===c.id?'#FFAA00':'transparent',display:'flex',alignItems:'center',justifyContent:'center',flexShrink:0}}>{sel===c.id&&<div style={{width:8,height:8,borderRadius:'50%',background:'#000'}}/>}</div></div>))}</div>
              )}
            </div>
            <div style={{background:'rgba(255,170,0,0.04)',border:'1px solid rgba(255,170,0,0.15)',borderRadius:10,padding:'10px 14px',fontSize:11,color:'rgba(255,170,0,0.7)',lineHeight:1.6}}>â„¹ï¸ Ta vidÃ©o sera examinÃ©e et apparaÃ®tra dans la compÃ©tition dÃ¨s son approbation.</div>
            {err&&<div style={{background:'rgba(248,113,113,0.1)',border:'1px solid rgba(248,113,113,0.25)',borderRadius:10,padding:'10px 14px',fontSize:13,color:'#f87171'}}>âš ï¸ {err}</div>}
            <div style={{display:'flex',gap:10,justifyContent:'flex-end',paddingTop:4}}><button onClick={onClose} style={btnSecondary}>Annuler</button><button onClick={submit} disabled={load||!sel||open.length===0} style={{...btnPrimary,opacity:(!sel||open.length===0)?0.4:1}}>{load?'â³ Soumissionâ€¦':'âœ… Soumettre pour validation'}</button></div>
          </div>
        )}
      </div>
    </div>
  ...
  return (
    <div onClick={onClose} style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.75)',backdropFilter:'blur(4px)',display:'flex',alignItems:'center',justifyContent:'center',zIndex:999,padding:16}}>
      <div onClick={e=>e.stopPropagation()} style={{background:'#12121e',border:'1px solid rgba(255,170,0,0.25)',borderRadius:20,width:'100%',maxWidth:480,overflow:'hidden'}}>
        <div style={{background:'linear-gradient(135deg,rgba(255,170,0,0.1),rgba(255,107,0,0.06))',borderBottom:'1px solid rgba(255,170,0,0.15)',padding:'16px 20px',display:'flex',alignItems:'center',justifyContent:'space-between'}}><div><div style={{fontSize:10,color:'#FFAA00',fontWeight:700,letterSpacing:'.1em',marginBottom:3}}>COMPÃ‰TITION</div><div style={{fontSize:17,fontWeight:800,color:'#fff',fontFamily:'Syne, sans-serif'}}>Inscrire une vidÃ©o ðŸ†</div></div><button onClick={onClose} style={{width:28,height:28,borderRadius:'50%',background:'rgba(255,255,255,0.06)',border:'1px solid rgba(255,255,255,0.1)',color:'rgba(255,255,255,0.5)',fontSize:14,cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center'}}>âœ•</button></div>
        {done?(<div style={{padding:40,textAlign:'center'}}><div style={{fontSize:48,marginBottom:12}}>ðŸŽ‰</div><p style={{color:'#4ade80',fontWeight:700,fontSize:15}}>Inscription enregistrÃ©e !</p></div>):(
          <div style={{padding:'18px 20px',display:'flex',flexDirection:'column',gap:14}}>
            {!preSelectedVideoId&&approvedVideos.length>0&&(<div><label style={{display:'block',fontSize:12,fontWeight:600,color:'rgba(255,255,255,0.5)',marginBottom:8,textTransform:'uppercase',letterSpacing:'.5px'}}>VidÃ©o approuvÃ©e</label><div style={{display:'flex',flexDirection:'column',gap:6}}>{approvedVideos.map(v=>(<div key={v.id} onClick={()=>setSelV(v.id)} style={{padding:'10px 14px',borderRadius:12,border:`1px solid ${selV===v.id?'#FFAA00':'rgba(255,255,255,0.1)'}`,background:selV===v.id?'rgba(255,170,0,0.08)':'rgba(255,255,255,0.03)',cursor:'pointer',display:'flex',alignItems:'center',gap:10}}><span style={{fontSize:20}}>{DISC_EMOJI[v.discipline??'']??'ðŸŽ¬'}</span><div style={{flex:1}}><div style={{fontSize:13,fontWeight:600,color:'#fff'}}>{v.title}</div><div style={{fontSize:11,color:'rgba(255,255,255,0.4)'}}>{DISC_FR[v.discipline??'']??v.discipline}</div></div>{selV===v.id&&<span style={{color:'#FFAA00',fontSize:16}}>âœ“</span>}</div>))}</div></div>)}
            {preSelectedVideoId&&(<div style={{padding:'10px 14px',borderRadius:12,border:'1px solid rgba(255,170,0,0.3)',background:'rgba(255,170,0,0.06)'}}><div style={{fontSize:11,color:'#FFAA00',fontWeight:700,marginBottom:3}}>VIDÃ‰O SÃ‰LECTIONNÃ‰E</div><div style={{fontSize:13,color:'#fff'}}>{approvedVideos.find(v=>v.id===preSelectedVideoId)?.title??preSelectedVideoId}</div></div>)}
            <div><label style={{display:'block',fontSize:12,fontWeight:600,color:'rgba(255,255,255,0.5)',marginBottom:8,textTransform:'uppercase',letterSpacing:'.5px'}}>CompÃ©tition ouverte</label>{open.length===0?(<p style={{fontSize:13,color:'rgba(255,255,255,0.4)',fontStyle:'italic'}}>Aucune compÃ©tition ouverte.</p>):(<div style={{display:'flex',flexDirection:'column',gap:6}}>{open.map(c=>(<div key={c.id} onClick={()=>setSelC(c.id)} style={{padding:'10px 14px',borderRadius:12,border:`1px solid ${selC===c.id?'#FFAA00':'rgba(255,255,255,0.1)'}`,background:selC===c.id?'rgba(255,170,0,0.08)':'rgba(255,255,255,0.03)',cursor:'pointer',display:'flex',alignItems:'center',gap:10}}><span style={{fontSize:20}}>{DISC_EMOJI[c.discipline]??'ðŸ†'}</span><div style={{flex:1}}><div style={{fontSize:13,fontWeight:600,color:'#fff'}}>{c.title}</div><div style={{fontSize:11,color:'rgba(255,255,255,0.4)'}}>{DISC_FR[c.discipline]??c.discipline} Â· {c.comp_type}</div></div>{selC===c.id&&<span style={{color:'#FFAA00',fontSize:16}}>âœ“</span>}</div>))}</div>)}</div>
            {err&&<div style={{background:'rgba(248,113,113,0.1)',border:'1px solid rgba(248,113,113,0.25)',borderRadius:10,padding:'10px 14px',fontSize:13,color:'#f87171'}}>âš ï¸ {err}</div>}
            <div style={{display:'flex',gap:10,justifyContent:'flex-end',paddingTop:4}}><button onClick={onClose} style={btnSecondary}>Annuler</button><button onClick={submit} disabled={load||!selV||!selC||approvedVideos.length===0} style={{...btnPrimary,opacity:(!selV||!selC||approvedVideos.length===0)?0.5:1}}>{load?'â³ Inscriptionâ€¦':"ðŸ† S'inscrire"}</button></div>
          </div>
        )}
      </div>
    </div>
...
// âœ… icon prop changÃ© de string Ã  React.ReactNode pour supporter <StarRed />
  ...
  return (
    <div>
      <div style={{background:'linear-gradient(135deg,rgba(255,170,0,0.1),rgba(255,107,0,0.06))',border:'1px solid rgba(255,170,0,0.3)',borderRadius:16,padding:'16px 18px',display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:12,gap:16}}>
        <div style={{display:'flex',alignItems:'center',gap:12}}><span style={{fontSize:24}}>ðŸ’°</span><div><div style={{fontSize:14,fontWeight:700,color:'#FFAA00',marginBottom:2}}>Montant encaissÃ©</div><div style={{fontSize:11,color:'rgba(255,255,255,0.4)'}}>Total des votes reÃ§us Â· Visible par tous</div></div></div>
      ...
      <div style={{display:'flex',flexDirection:'column',gap:8,marginBottom:16}}>
        <PrivacyRow icon="ðŸ‘¤" label="Nom affichÃ©"         desc="Ton nom sur ton profil public"            on={privacy.name_visible}     onToggle={()=>toggle('name_visible')}/>
        <PrivacyRow icon="ðŸ–¼ï¸" label="Photo de profil"    desc="Ton avatar visible par tous"              on={privacy.photo_visible}    onToggle={()=>toggle('photo_visible')}/>
        <PrivacyRow icon="ðŸ“ž" label="NumÃ©ro de tÃ©lÃ©phone" desc="Permettre aux fans de te contacter"      on={privacy.phone_visible}    onToggle={()=>toggle('phone_visible')}/>
        <PrivacyRow icon="ðŸ“§" label="Adresse email"       desc="Ton email affichÃ© sur ton profil"        on={privacy.email_visible}    onToggle={()=>toggle('email_visible')}/>
        {/* âœ… â­ â†’ <StarRed /> */}
        <PrivacyRow icon={<StarRed />} label="Score de votes" desc="Total de votes reÃ§us visible sur profil" on={privacy.score_visible} onToggle={()=>toggle('score_visible')}/>
        <PrivacyRow icon="ðŸ“" label="Localisation"        desc="Ta ville ou pays sur ton profil"         on={privacy.location_visible} onToggle={()=>toggle('location_visible')}/>
      </div>
      <div style={{display:'flex',alignItems:'center',justifyContent:'flex-end',gap:12}}>
        {saved&&<span style={{fontSize:12,color:'#4ade80',fontWeight:600}}>âœ“ EnregistrÃ©</span>}
        <button onClick={save} disabled={saving} style={{...btnPrimary,opacity:saving?0.5:1}}>{saving?'â³ Enregistrementâ€¦':'ðŸ’¾ Enregistrer'}</button>
      </div>
    </div>
  );
}
...
function CandidateCard({cand,isWinner,isLoser,voteState,loading,contest,isFav,onToggleFav,onVote}:{cand:Candidate;isWinner:boolean;isLoser:boolean;voteState:VoteState;loading:boolean;contest:Contest;isFav:boolean;onToggleFav:()=>void;onVote:()=>void}) {
  ...
  return (
    <div style={{flex:1,background:bg,border:`1px solid ${bc}`,borderRadius:'16px',padding:'16px 12px',display:'flex',flexDirection:'column',alignItems:'center',gap:'12px',position:'relative'}}>
      {isWinner&&<div style={{position:'absolute',top:'-10px',left:'50%',transform:'translateX(-50%)',background:'#4ade80',color:'#000',fontSize:'10px',fontWeight:700,padding:'2px 10px',borderRadius:'20px',whiteSpace:'nowrap'}}>âœ“ Votre vote</div>}
      ...
      <div onClick={()=>vid&&window.open(`/watch/${vid}`,'_blank')} style={{width:'100%',aspectRatio:'16/9',background:'rgba(255,255,255,.06)',borderRadius:'12px',overflow:'hidden',display:'flex',alignItems:'center',justifyContent:'center',position:'relative',cursor:'pointer'}}>
        {cand.video?.thumbnail_url?<img src={cand.video.thumbnail_url} alt={cand.name} style={{width:'100%',height:'100%',objectFit:'cover'}}/>:<div style={{fontSize:'32px'}}>{DISC_EMOJI[contest.discipline]||'ðŸŽ­'}</div>}
      ...
      <div style={{textAlign:'center',width:'100%'}}>
        ...
        {cand.track_title&&<div style={{fontSize:'11px',color:'rgba(255,255,255,.4)'}}>ðŸŽµ {cand.track_title}{cand.track_artist&&` â€” ${cand.track_artist}`}</div>}
      </div>
      ...
      <button onClick={handleVote} disabled={voteState.voted||loading||contest.status!=='active'||!vid} style={{width:'100%',padding:'11px',background:voteState.voted?(isWinner?'rgba(74,222,128,.15)':'rgba(255,255,255,.05)'):'#FFAA00',border:voteState.voted?(isWinner?'0.5px solid rgba(74,222,128,.3)':'0.5px solid rgba(255,255,255,.1)'):'none',borderRadius:'12px',fontFamily:'Syne,sans-serif',fontSize:'13px',fontWeight:700,color:voteState.voted?(isWinner?'#4ade80':'rgba(255,255,255,.3)'):'#000',cursor:voteState.voted||loading||!vid?'not-allowed':'pointer',transform:pop?'scale(1.05)':'scale(1)',transition:'all .2s'}}>
        {loading?'...':voteState.voted?(isWinner?'âœ“ VotÃ©':'Non choisi'):!vid?'Pas de vidÃ©o':'ðŸ‘ Voter Â· 10 F CFA'}
      </button>
    </div>
  );
}
...
function ContestCard({contest,userBalance,onVoted,isFavContest,favCandidates,onToggleFavContest,onToggleFavCandidate}:{contest:Contest;userBalance:number;onVoted:(nb:number)=>void;isFavContest:boolean;favCandidates:string[];onToggleFavContest:()=>void;onToggleFavCandidate:(id:string)=>void}) {
  ...
  async function handleVote(videoId:string,name:string){if(vs.voted){showMsg('Vous avez dÃ©jÃ  votÃ©.','info');return;}if(userBalance<100){showMsg('Solde insuffisant.','error');return;}setLoad(true);setVs({voted:true,votedVideoId:videoId});setLc(p=>p.map(c=>c.video?.id===videoId?{...c,votes:c.votes+1}:c));try{const res=await fetch(`${API}/votes`,{method:'POST',headers:{'Content-Type':'application/json',Authorization:`Bearer ${localStorage.getItem('dkdk_token')}`},body:JSON.stringify({video_id:videoId})});const r=await res.json();if(!res.ok)throw new Error(r.error||r.code||'ERR');onVoted(r.new_balance??r.balance??userBalance-100);showMsg(`âœ“ Vote pour ${name} Â· -10 F CFA`,'success');}catch(e:any){setVs({voted:false});setLc(contest.candidates);const EM:Record<string,string>={INSUFFICIENT_BALANCE:'Solde insuffisant.',ALREADY_VOTED:'DÃ©jÃ  votÃ©.',CONTEST_NOT_ACTIVE:'Concours terminÃ©.',TOKEN_MISSING:'Connectez-vous.'};showMsg(EM[(e as any).message]??'Erreur.','error');}finally{setLoad(false);}  }
  ...
  const bbg=vs.voted||isOpen?'rgba(74,222,128,.15)':'rgba(255,255,255,.05)';const bco=vs.voted||isOpen?'#4ade80':'rgba(255,255,255,.3)';const blb=vs.voted?'âœ“ VotÃ©':isOpen?'â— Vote ouvert':'BientÃ´t';
  return (
    <div style={{background:'rgba(255,255,255,.04)',border:'0.5px solid rgba(255,255,255,.08)',borderRadius:'20px',overflow:'hidden',marginBottom:'16px'}}>
      <div style={{padding:'14px 16px',borderBottom:'0.5px solid rgba(255,255,255,.06)',display:'flex',alignItems:'center',justifyContent:'space-between',background:'rgba(255,255,255,.02)'}}>
        <div><div style={{fontSize:'15px',fontWeight:700,color:'#fff',fontFamily:'Syne,sans-serif',marginBottom:'3px'}}>{DISC_EMOJI[contest.discipline]||'ðŸŽ­'} {contest.title}</div><div style={{display:'flex',gap:'6px',flexWrap:'wrap'}}>{[DISC_FR[contest.discipline]||contest.discipline,contest.comp_type,`â³ ${days}j restants`].map(l=><span key={l} style={{fontSize:'10px',color:'rgba(255,255,255,.4)',background:'rgba(255,255,255,.06)',padding:'2px 8px',borderRadius:'20px'}}>{l}</span>)}</div></div>
      ...
      <div style={{...card, background: 'linear-gradient(135deg, rgba(126, 3, 128, 0.52), rgba(237,7,15))', border: 'none'}}>
        <div style={{display:'flex',alignItems:'flex-start',justifyContent:'space-between',marginBottom:16}}>
          <div style={{display:'flex',alignItems:'center',gap:14}}>
            ...
            <div><div style={{fontSize:17,fontWeight:800,color:'#fff',fontFamily:'Syne,sans-serif',marginBottom:3}}>{profile?.name??'â€”'}</div><div style={{fontSize:12,color:'rgba(255,255,255,0.85)'}}>{profile?.email}</div>{country&&<div style={{fontSize:12,color:'rgba(255,255,255,0.5)',marginTop:3}}>{country.flag} {country.name}</div>}</div>
          </div>
          <button onClick={onEditProfile} style={{...btnSecondary,padding:'7px 14px',fontSize:12,color:'#fff',border:'1px solid rgba(255,255,255,0.4)',background:'rgba(0,0,0,0.45)'}}>âœï¸ Modifier</button>
        </div>
        <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:10}}>
          {[{val:videoCount,label:'VidÃ©os ajoutÃ©es',color:'#f0f0f0'},{val:votesEmis,label:'Votes Ã©mis',color:'#f0f0f0'},{val:`${totalEarned.toLocaleString('fr-FR')} F`,label:'Gains reÃ§us',color:'#FFAA00'}].map(s=>(<div key={s.label} style={{background:'rgba(0,0,0,0.35)',borderRadius:12,padding:'14px',textAlign:'center'}}><div style={{fontSize:18,fontWeight:800,color:s.color,fontFamily:'Syne,sans-serif',lineHeight:1}}>{s.val}</div><div style={{fontSize:11,color:'#fff',fontWeight:600,marginTop:4,lineHeight:1.3}}>{s.label}</div></div>))}
        </div>
      </div>
      <div style={{...card,background:'linear-gradient(135deg, rgba(126, 3, 128, 0.52), rgba(237,7,15))',border:'none',}}>
...
function EditProfileModal({profile,onClose,onSaved}:{profile:UserProfile;onClose:()=>void;onSaved:(p:UserProfile)=>void}) {
  ...
  return (
    <div onClick={onClose} style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.75)',backdropFilter:'blur(4px)',display:'flex',alignItems:'center',justifyContent:'center',zIndex:999,padding:16}}>
      <div onClick={e=>e.stopPropagation()} style={{background:'#12121e',border:'1px solid rgba(255,170,0,0.25)',borderRadius:20,width:'100%',maxWidth:460,overflow:'hidden'}}>
        <div style={{background:'linear-gradient(135deg,rgba(255,170,0,0.1),rgba(255,107,0,0.06))',borderBottom:'1px solid rgba(255,170,0,0.15)',padding:'16px 20px',display:'flex',alignItems:'center',justifyContent:'space-between'}}><div style={{fontSize:17,fontWeight:800,color:'#fff',fontFamily:'Syne,sans-serif'}}>âœï¸ Modifier mon profil</div><button onClick={onClose} style={{width:28,height:28,borderRadius:'50%',background:'rgba(255,255,255,0.06)',border:'1px solid rgba(255,255,255,0.1)',color:'rgba(255,255,255,0.5)',fontSize:14,cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center'}}>âœ•</button></div>
        {done?(<div style={{padding:40,textAlign:'center'}}><div style={{fontSize:44,marginBottom:10}}>âœ…</div><p style={{color:'#4ade80',fontWeight:700}}>Profil mis Ã  jour !</p></div>):(
          <div style={{padding:'20px',display:'flex',flexDirection:'column',gap:14}}>
            <div style={{display:'flex',alignItems:'center',gap:14,marginBottom:4}}>{form.photo_url?<img src={form.photo_url} alt="avatar" style={{width:52,height:52,borderRadius:'50%',objectFit:'cover',border:'2px solid rgba(255,170,0,0.3)'}}/>:<div style={{width:52,height:52,borderRadius:'50%',background:'rgba(255,170,0,0.15)',border:'2px solid rgba(255,170,0,0.3)',display:'flex',alignItems:'center',justifyContent:'center',fontSize:20,fontWeight:700,color:'#FFAA00'}}>{form.name.split(' ').map(w=>w[0]).join('').toUpperCase().slice(0,2)||'?'}</div>}<div style={{flex:1}}><label style={lbl}>Photo (URL)</label><input style={inp} type="url" placeholder="https://â€¦" value={form.photo_url} onChange={e=>setForm(f=>({...f,photo_url:e.target.value}))}/></div></div>
            <div><label style={lbl}>Nom affichÃ© *</label><input style={inp} type="text" value={form.name} onChange={e=>setForm(f=>({...f,name:e.target.value}))}/></div>
            <div><label style={lbl}>Pays</label><select style={{...inp,cursor:'pointer'}} value={form.country} onChange={e=>setForm(f=>({...f,country:e.target.value}))}><option value="">â€” SÃ©lectionner â€”</option>{COUNTRIES.map(c=><option key={c.code} value={c.code}>{c.flag} {c.name}</option>)}</select></div>
            <div><label style={lbl}>Bio (facultatif)</label><textarea style={{...inp,resize:'vertical',minHeight:70}} placeholder="Quelques mots sur toiâ€¦" value={form.bio} onChange={e=>setForm(f=>({...f,bio:e.target.value}))} maxLength={200}/><div style={{textAlign:'right',fontSize:10,color:'rgba(255,255,255,0.3)',marginTop:3}}>{form.bio.length}/200</div></div>
            {err&&<div style={{background:'rgba(248,113,113,0.1)',border:'1px solid rgba(248,113,113,0.25)',borderRadius:10,padding:'10px 14px',fontSize:13,color:'#f87171'}}>âš ï¸ {err}</div>}
            <div style={{display:'flex',gap:10,justifyContent:'flex-end',paddingTop:4}}><button onClick={onClose} style={btnSecondary}>Annuler</button><button onClick={save} disabled={saving} style={{...btnPrimary,opacity:saving?0.6:1}}>{saving?'â³ Enregistrementâ€¦':'ðŸ’¾ Enregistrer'}</button></div>
          </div>
        )}
      </div>
    </div>
...
function MesVideosSection({videos,loading,contests,onRefresh,router}:{videos:UserVideo[];loading:boolean;contests:Contest[];onRefresh:()=>void;router:any}) {
  ...
  return (
    <div>
      <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:14}}><div style={{fontSize:13,color:'rgba(255,255,255,0.4)'}}>{videos.length} vidÃ©o{videos.length!==1?'s':''}</div><button onClick={()=>router.push('/submit')} style={btnPrimary}>ðŸŽ¬ + Ajouter une vidÃ©o</button></div>
      {loading?(<div style={{textAlign:'center',padding:'40px 20px',color:'rgba(255,255,255,0.3)',fontSize:13}}>â³ Chargementâ€¦</div>):videos.length===0?(
        <div style={{...card,textAlign:'center',padding:'40px 20px'}}><div style={{fontSize:40,marginBottom:12}}>ðŸŽ¬</div><p style={{color:'rgba(255,255,255,0.5)',fontSize:14,marginBottom:16}}>Tu n'as pas encore ajoutÃ© de vidÃ©o.</p><button onClick={()=>router.push('/submit')} style={btnPrimary}>Ajouter ma premiÃ¨re vidÃ©o</button></div>
      ):(
        <>
          {drafts.length>0&&(<div style={{marginBottom:8}}><div style={{fontSize:11,fontWeight:700,color:'rgba(255,255,255,0.3)',letterSpacing:'.08em',marginBottom:8,textTransform:'uppercase'}}>ðŸ“ Brouillons â€” Ã  soumettre pour validation</div>{drafts.map(v=>(<div key={v.id} style={{...card,border:'1px solid rgba(255,255,255,0.12)'}}><div style={{display:'flex',alignItems:'flex-start',gap:12}}><div style={{width:46,height:46,borderRadius:10,background:'rgba(255,255,255,0.06)',display:'flex',alignItems:'center',justifyContent:'center',fontSize:20,flexShrink:0}}>{DISC_EMOJI[v.discipline??'']??'ðŸŽ¬'}</div><div style={{flex:1,minWidth:0}}><div style={{display:'flex',alignItems:'center',gap:8,marginBottom:4,flexWrap:'wrap'}}><span style={{fontSize:14,fontWeight:700,color:'#fff',overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{v.title}</span><StatusBadge status="draft"/></div><div style={{fontSize:11,color:'rgba(255,255,255,0.4)',marginBottom:10}}>{DISC_FR[v.discipline??'']??v.discipline} Â· Non soumise Ã  la modÃ©ration</div><button onClick={()=>setSubId(v.id)} style={{background:'linear-gradient(135deg,#FFAA00,#FF6B00)',border:'none',borderRadius:50,padding:'7px 16px',fontSize:12,fontWeight:700,color:'#000',cursor:'pointer',fontFamily:'DM Sans,sans-serif'}}>ðŸ† Soumettre pour validation</button></div></div></div>))}</div>)}
          {others.map(v=>(
            <div key={v.id} style={card}>
              <div style={{display:'flex',alignItems:'flex-start',gap:12}}>
                <div style={{width:46,height:46,borderRadius:10,background:'rgba(255,255,255,0.06)',display:'flex',alignItems:'center',justifyContent:'center',fontSize:20,flexShrink:0}}>{DISC_EMOJI[v.discipline??'']??'ðŸŽ¬'}</div>
                <div style={{flex:1,minWidth:0}}>
                  ...
                  <div style={{fontSize:11,color:'rgba(255,255,255,0.4)',marginBottom:v.status==='rejected'||v.status==='approved'?8:0}}>
                    ...
                    {v.views!==undefined&&` Â· ðŸ‘ ${v.views.toLocaleString('fr-FR')}`}
                    {v.vote_count!==undefined&&v.vote_count>0&&<> Â· <StarRed /> {v.vote_count}</>}
                  </div>
                  ...
                  {v.status==='approved'&&<div style={{display:'flex',gap:8,flexWrap:'wrap'}}><button onClick={()=>router.push(`/watch/${v.id}`)} style={sm('rgba(255,255,255,0.6)','transparent','rgba(255,255,255,0.15)')}>â–¶ Regarder</button><button onClick={()=>setInsId(v.id)} style={sm('#FFAA00','rgba(255,170,0,0.1)','rgba(255,170,0,0.3)')}>ðŸ† Inscrire en compÃ©tition</button></div>}
                </div>
              </div>
            </div>
          ))}
...
function CompetitionsSection({contests,balance,approvedVideos,onVoted,favContests,favCandidates,onToggleFavContest,onToggleFavCandidate}:{contests:Contest[];balance:number;approvedVideos:UserVideo[];onVoted:(nb:number)=>void;favContests:string[];favCandidates:string[];onToggleFavContest:(id:string)=>void;onToggleFavCandidate:(id:string)=>void}) {
  ...
  return (
    <div>
      <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:14,flexWrap:'wrap',gap:8}}><div style={{fontSize:13,color:'rgba(255,255,255,0.4)'}}>{filtered.length} compÃ©tition{filtered.length!==1?'s':''}</div><button onClick={()=>setShowIns(true)} style={{...btnPrimary,padding:'8px 16px',fontSize:12}}>ðŸ† Inscrire ma vidÃ©o dans une compÃ©tition</button></div>
      ...
      {filtered.length===0?(<div style={{textAlign:'center',padding:'40px 20px',color:'rgba(255,255,255,0.3)',fontSize:13}}>{contests.length===0?'Aucune compÃ©tition active pour le moment.':'Aucune compÃ©tition dans cette catÃ©gorie.'}</div>):filtered.map(c=><ContestCard key={c.id} contest={c} userBalance={balance} onVoted={onVoted} isFavContest={favContests.includes(c.id)} favCandidates={favCandidates} onToggleFavContest={()=>onToggleFavContest(c.id)} onToggleFavCandidate={onToggleFavCandidate}/>)}
...
function EducationSection({router}:{router:any}) {
  const MATIERES=['ðŸŒ Langues du monde','ðŸ“œ Histoire & GÃ©ographie','ðŸ”¬ Sciences & Vie','ðŸŽ¨ Art & Culture du monde','ðŸŒ¿ Agriculture','ðŸ’» Informatique','ðŸŽµ Musique du monde','ðŸ½ï¸ Gastronomie du monde'];
  return (
    <div>
      <div style={{background:'linear-gradient(135deg,rgba(255,170,0,0.08),rgba(255,107,0,0.04))',border:'1px solid rgba(255,170,0,0.2)',borderRadius:18,padding:'20px',marginBottom:14,textAlign:'center'}}>
        <div style={{fontSize:40,marginBottom:10}}>ðŸ“š</div>
        <div style={{fontFamily:'Syne,sans-serif',fontWeight:800,fontSize:18,color:'#fff',marginBottom:6}}>Ã‰ducation & Savoirs</div>
        <div style={{fontSize:13,color:'rgba(255,255,255,0.4)',marginBottom:16,lineHeight:1.6}}>Explorez 21 matiÃ¨res.<br/>CrÃ©ez et partagez du contenu Ã©ducatif.</div>
        <div style={{display:'flex',gap:10,justifyContent:'center',flexWrap:'wrap'}}>
          <button onClick={()=>router.push('/education')} style={btnPrimary}>ðŸ“– Explorer les matiÃ¨res</button>
          <button onClick={()=>router.push('/education/creer')} style={{...btnPrimary,background:'rgba(255,170,0,0.15)',color:'#FFAA00',border:'1px solid rgba(255,170,0,0.3)'}}>âœï¸ CrÃ©er du contenu</button>
        </div>
      </div>
      <div style={{display:'grid',gridTemplateColumns:'repeat(2,1fr)',gap:10}}>
        ...
        <div onClick={()=>router.push('/education')} style={{background:'rgba(255,170,0,0.05)',border:'1px solid rgba(255,170,0,0.15)',borderRadius:12,padding:'12px 14px',fontSize:13,color:'rgba(255,170,0,0.6)',cursor:'pointer',gridColumn:'span 2',textAlign:'center'}}>+13 autres matiÃ¨res â†’</div>
      </div>
    </div>
  );
}
...
function FinancesSection({balance,totalEarned,router}:{balance:number;totalEarned:number;router:any}) {
  return (
    <div>
      <div style={{...card,background:'linear-gradient(135deg,rgba(255,170,0,0.08),rgba(255,107,0,0.04))',border:'1px solid rgba(255,170,0,0.2)',marginBottom:12}}><div style={{fontSize:11,color:'rgba(255,170,0,0.6)',fontWeight:700,letterSpacing:'.08em',marginBottom:6}}>SOLDE WALLET</div><div style={{fontSize:32,fontWeight:800,color:'#FFAA00',fontFamily:'Syne,sans-serif',marginBottom:4}}>{balance.toLocaleString('fr-FR')} F CFA</div><div style={{fontSize:12,color:'rgba(255,255,255,0.4)'}}>{Math.floor(balance/100)} vote{Math.floor(balance/100)!==1?'s':''} disponibles</div></div>
      <div style={{...card,marginBottom:12}}><div style={{display:'flex',alignItems:'center',justifyContent:'space-between'}}><div><div style={{fontSize:13,fontWeight:600,color:'#fff',marginBottom:3}}>ðŸ’° Total encaissÃ©</div><div style={{fontSize:11,color:'rgba(255,255,255,0.4)'}}>Votes reÃ§us sur tes vidÃ©os Â· Visible par tous</div></div><div style={{textAlign:'right'}}><div style={{fontSize:20,fontWeight:800,color:'#FFAA00',fontFamily:'Syne,sans-serif'}}>{totalEarned.toLocaleString('fr-FR')} F</div><span style={{background:'rgba(255,170,0,0.12)',border:'1px solid rgba(255,170,0,0.25)',borderRadius:20,padding:'2px 8px',fontSize:10,color:'#FFAA00',fontWeight:700}}>PUBLIC</span></div></div></div>
      <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:12}}>
        <div style={{...card,textAlign:'center'}}><div style={{fontSize:28,marginBottom:10}}>âš¡</div><div style={{fontSize:14,fontWeight:700,color:'#fff',marginBottom:6}}>Recharger</div><div style={{fontSize:11,color:'rgba(255,255,255,0.4)',marginBottom:14,lineHeight:1.5}}>Acheter des Ã©toiles et cÅ“urs via FedaPay, MTN, Moov.</div><button onClick={()=>router.push('/recharge')} style={btnPrimary}>Recharger mon compte</button></div>
        <div style={{...card,textAlign:'center'}}><div style={{fontSize:28,marginBottom:10}}>ðŸ’¸</div><div style={{fontSize:14,fontWeight:700,color:'#fff',marginBottom:6}}>Retrait</div><div style={{fontSize:11,color:'rgba(255,255,255,0.4)',marginBottom:14,lineHeight:1.5}}>Retirer tes gains vers ton Mobile Money ou compte bancaire.</div><button onClick={()=>router.push('/retrait')} style={{...btnSecondary,display:'inline-block'}}>Faire un retrait</button></div>
      </div>
    </div>
  );
}
    ...
    <div style={{minHeight:'100vh',background:'#0a0a0f',color:'#f0f0f0',fontFamily:'DM Sans,sans-serif',paddingBottom:60}}>
      <div style={{position:'sticky',top:0,zIndex:100,background:'rgba(8,8,15,0.95)',borderBottom:'1px solid rgba(255,255,255,0.08)',padding:'0 20px 0 0',height:60,display:'flex',alignItems:'center',justifyContent:'space-between'}}>
        ...
        <div style={{display:'flex',alignItems:'center',gap:10}}>
          <TranslateWidget/>
          <div style={{display:'flex',alignItems:'center',gap:6,background:'rgba(255,170,0,0.08)',border:'1px solid rgba(246,245,244,0.2)',borderRadius:20,padding:'5px 12px'}}><span style={{fontSize:13}}>ðŸ’°</span><span style={{fontSize:13,fontWeight:700,color:'#FFAA00'}}>{balance.toLocaleString('fr-FR')} F</span></div>
          <div onClick={()=>setActiveTab('dashboard')} style={{width:36,height:36,borderRadius:'50%',background:'linear-gradient(135deg,#FF6B00,#FFD700)',display:'flex',alignItems:'center',justifyContent:'center',fontSize:18,cursor:'pointer',flexShrink:0}}>ðŸ‘¤</div>
        </div>
      </div>
      ...
      <div style={{maxWidth:660,margin:'0 auto',padding:'20px 16px'}}>
        ...
        {activeTab==='competitions'&&(loading?<div style={{textAlign:'center',padding:'40px',color:'rgba(255,255,255,0.3)'}}>â³ Chargementâ€¦</div>:<CompetitionsSection contests={contests} balance={balance} approvedVideos={approved} onVoted={nb=>setBalance(nb)} favContests={favContests} favCandidates={favCandidates} onToggleFavContest={toggleFavContest} onToggleFavCandidate={toggleFavCandidate}/>)}
        ...
        {activeTab==='settings'&&(
          <div>
            <div style={card}><div style={{fontSize:15,fontWeight:700,color:'#fff',marginBottom:16,fontFamily:'Syne,sans-serif'}}>ðŸ”’ ConfidentialitÃ©</div><ConfidentialiteSection earnings={totalEarned}/></div>
            <div style={{...card,marginTop:12}}><div style={{fontSize:15,fontWeight:700,color:'#fff',marginBottom:12,fontFamily:'Syne,sans-serif'}}>âš™ï¸ Compte</div><button onClick={handleLogout} style={{background:'rgba(248,113,113,0.1)',border:'1px solid rgba(248,113,113,0.25)',borderRadius:50,padding:'10px 20px',fontSize:13,fontWeight:600,color:'#f87171',cursor:'pointer',fontFamily:'DM Sans,sans-serif'}}>ðŸšª Se dÃ©connecter</button></div>
          </div>
        )}
      </div>
