...
function Toggle({on,onToggle}:{on:boolean;onToggle:()=>void}) {
  return <div onClick={onToggle} style={{width:48,height:26,borderRadius:13,background:on?'linear-gradient(135deg,#FF6B00,#FFD700)':'var(--line-strong)',display:'flex',alignItems:'center',padding:3,cursor:'pointer',flexShrink:0,transition:'background 0.25s'}}><div style={{width:20,height:20,borderRadius:'50%',background:'#fff',marginLeft:on?'auto':0,transition:'margin 0.25s',boxShadow:'0 1px 3px rgba(0,0,0,0.3)'}}/></div>;
}
...
function MesVideosSection({videos,loading,onRefresh,router}:{videos:UserVideo[];loading:boolean;onRefresh:()=>void;router:any}) {
  const others=videos.filter(v=>v.status!=='draft');
      ...
      <div style={{maxWidth:660,margin:'0 auto',padding:'20px 16px'}}>
        ...
        {activeTab==='settings'&&(
          <div>
            ...
            <div style={{...card,marginTop:12}}><div style={{fontSize:15,fontWeight:700,color:'var(--ink)',marginBottom:12,fontFamily:'Syne,sans-serif'}}>⚙️ Compte</div><button onClick={handleLogout} style={{background:'#e10600',border:'none',borderRadius:50,padding:'11px 22px',fontSize:13,fontWeight:800,color:'#fff',cursor:'pointer',fontFamily:'DM Sans,sans-serif',boxShadow:'0 2px 8px rgba(225,6,0,0.35)'}}>🚪 Se déconnecter</button></div>
          </div>
        )}
      </div>
