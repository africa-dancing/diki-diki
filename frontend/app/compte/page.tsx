...
function MesVideosSection({videos,loading,contests,onRefresh,router}:{videos:UserVideo[];loading:boolean;contests:Contest[];onRefresh:()=>void;router:any}) {
  ...
  return (
    <div>
      <div style={{background:'linear-gradient(135deg,rgba(126,3,128,0.52),rgba(237,7,15))',borderRadius:18,padding:'22px 20px',marginBottom:18,textAlign:'center'}}><div style={{fontSize:38,marginBottom:8}}>🎬</div><div style={{fontFamily:'Syne,sans-serif',fontWeight:800,fontSize:20,color:'#fff',marginBottom:6}}>Mes vidéos</div><div style={{fontSize:13,color:'rgba(255,255,255,0.85)',lineHeight:1.6,marginBottom:16}}>{videos.length} vidéo{videos.length!==1?'s':''} · Gère tes prestations et inscris-les en compétition</div><button onClick={()=>router.push('/submit')} style={btnPrimary}>🎬 + Ajouter une vidéo</button></div>
...
export default function ComptePage() {
  ...
  return (
    <div style={{minHeight:'100vh',background:'#0a0a0f',color:'#f0f0f0',fontFamily:'DM Sans,sans-serif',paddingBottom:60}}>
      <div style={{position:'fixed',top:0,left:0,right:0,zIndex:200,background:'rgba(8,8,15,0.95)',borderBottom:'1px solid rgba(255,255,255,0.08)',padding:'0 20px 0 0',height:56,display:'flex',alignItems:'center',justifyContent:'space-between'}}>
