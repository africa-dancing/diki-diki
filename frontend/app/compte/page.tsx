...
function StatusBadge({status}:{status:UserVideo['status']}) {
  const cfg = {
    draft:    {bg:'rgba(255,255,255,0.06)',color:'rgba(255,255,255,0.5)',border:'rgba(255,255,255,0.15)',label:'📝 Brouillon'},
    ...
    <div onClick={onClose} style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.78)',backdropFilter:'blur(4px)',display:'flex',alignItems:'center',justifyContent:'center',zIndex:999,padding:16}}>
      <div onClick={e=>e.stopPropagation()} style={{background:'#12121e',border:'1px solid rgba(255,170,0,0.25)',borderRadius:20,width:'100%',maxWidth:460,overflow:'hidden'}}>
        <div style={{background:'linear-gradient(135deg,rgba(255,170,0,0.1),rgba(255,107,0,0.06))',borderBottom:'1px solid rgba(255,170,0,0.15)',padding:'16px 20px',display:'flex',alignItems:'center',justifyContent:'space-between'}}>
          ...
          <button onClick={onClose} style={{width:28,height:28,borderRadius:'50%',background:'rgba(255,255,255,0.06)',border:'1px solid rgba(255,255,255,0.1)',color:'rgba(255,255,255,0.5)',fontSize:14,cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center'}}>✕</button>
        </div>
        {done?(<div style={{padding:40,textAlign:'center'}}><div style={{fontSize:44,marginBottom:12}}>🎉</div><p style={{color:'#4ade80',fontWeight:700,fontSize:15,marginBottom:6}}>Vidéo soumise pour validation !</p><p style={{color:'rgba(255,255,255,0.4)',fontSize:12,lineHeight:1.6}}>Notre équipe examinera ta vidéo sous 24 à 48h.</p></div>):(
          <div style={{padding:'18px 20px',display:'flex',flexDirection:'column',gap:14}}>
            <div style={{background:'rgba(255,255,255,0.04)',border:'1px solid rgba(255,255,255,0.08)',borderRadius:12,padding:'10px 14px',display:'flex',alignItems:'center',gap:10}}><span style={{fontSize:20}}>{DISC_EMOJI[video.discipline??'']??'🎬'}</span><div><div style={{fontSize:13,fontWeight:600,color:'#fff'}}>{video.title}</div><div style={{fontSize:11,color:'rgba(255,255,255,0.4)'}}>Brouillon → En attente de modération</div></div></div>
            <div>
              <label style={{display:'block',fontSize:12,fontWeight:600,color:'rgba(255,255,255,0.5)',marginBottom:8,textTransform:'uppercase',letterSpacing:'.5px'}}>Compétition * <span style={{color:'rgba(248,113,113,0.7)',fontSize:10,textTransform:'none',fontWeight:400}}>obligatoire</span></label>
  ...
  return (
    <div onClick={onClose} style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.75)',backdropFilter:'blur(4px)',display:'flex',alignItems:'center',justifyContent:'center',zIndex:999,padding:16}}>
      <div onClick={e=>e.stopPropagation()} style={{background:'#12121e',border:'1px solid rgba(255,170,0,0.25)',borderRadius:20,width:'100%',maxWidth:480,overflow:'hidden'}}>
        <div style={{background:'linear-gradient(135deg,rgba(255,170,0,0.1),rgba(255,107,0,0.06))',borderBottom:'1px solid rgba(255,170,0,0.15)',padding:'16px 20px',display:'flex',alignItems:'center',justifyContent:'space-between'}}><div><div style={{fontSize:10,color:'#FFAA00',fontWeight:700,letterSpacing:'.1em',marginBottom:3}}>COMPÉTITION</div><div style={{fontSize:17,fontWeight:800,color:'#fff',fontFamily:'Syne, sans-serif'}}>Inscrire une vidéo 🏆</div></div><button onClick={onClose} style={{width:28,height:28,borderRadius:'50%',background:'rgba(255,255,255,0.06)',border:'1px solid rgba(255,255,255,0.1)',color:'rgba(255,255,255,0.5)',fontSize:14,cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center'}}>✕</button></div>
  ...
  return (
    <div>
      <div style={{background:'linear-gradient(135deg,rgba(255,170,0,0.1),rgba(255,107,0,0.06))',border:'1px solid rgba(255,170,0,0.3)',borderRadius:16,padding:'16px 18px',display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:12,gap:16}}>
        <div style={{display:'flex',alignItems:'center',gap:12}}><span style={{fontSize:24}}>💰</span><div><div style={{fontSize:14,fontWeight:700,color:'#FFAA00',marginBottom:2}}>Montant encaissé</div><div style={{fontSize:11,color:'rgba(255,255,255,0.4)'}}>Total des votes reçus · Visible par tous</div></div></div>
  ...
  return (
    <div style={{background:'rgba(255,255,255,.04)',border:'0.5px solid rgba(255,255,255,.08)',borderRadius:'20px',overflow:'hidden',marginBottom:'16px'}}>
      <div style={{padding:'14px 16px',borderBottom:'0.5px solid rgba(255,255,255,.06)',display:'flex',alignItems:'center',justifyContent:'space-between',background:'rgba(255,255,255,.02)'}}>
        <div><div style={{fontSize:'15px',fontWeight:700,color:'#fff',fontFamily:'Syne,sans-serif',marginBottom:'3px'}}>{DISC_EMOJI[contest.discipline]||'🎭'} {contest.title}</div><div style={{display:'flex',gap:'6px',flexWrap:'wrap'}}>{[DISC_FR[contest.discipline]||contest.discipline,contest.comp_type,`⏳ ${days}j restants`].map(l=><span key={l} style={{fontSize:'10px',color:'rgba(255,255,255,.4)',background:'rgba(255,255,255,.06)',padding:'2px 8px',borderRadius:'20px'}}>{l}</span>)}</div></div>
...
function DashboardSection({profile,balance,votesEmis,totalEarned,videoCount,onEditProfile}:{profile:UserProfile|null;balance:number;votesEmis:number;totalEarned:number;videoCount:number;onEditProfile:()=>void}) {
  ...
  return (
    <div>
      <div style={card}>
        <div style={{display:'flex',alignItems:'flex-start',justifyContent:'space-between',marginBottom:16}}>
          <div style={{display:'flex',alignItems:'center',gap:14}}>
            ...
            <div><div style={{fontSize:17,fontWeight:800,color:'#fff',fontFamily:'Syne,sans-serif',marginBottom:3}}>{profile?.name??'—'}</div><div style={{fontSize:12,color:'rgb(251,250,250)'}}>{profile?.email}</div>{country&&<div style={{fontSize:12,color:'rgba(255,255,255,0.5)',marginTop:3}}>{country.flag} {country.name}</div>}</div>
          </div>
      ...
      <div style={{...card,background:'linear-gradient(135deg,rgb(249,3,3),rgb(247,3,3))',border:'1px solid rgb(249,247,247)'}}>
...
function EditProfileModal({profile,onClose,onSaved}:{profile:UserProfile;onClose:()=>void;onSaved:(p:UserProfile)=>void}) {
  ...
  return (
    <div onClick={onClose} style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.75)',backdropFilter:'blur(4px)',display:'flex',alignItems:'center',justifyContent:'center',zIndex:999,padding:16}}>
      <div onClick={e=>e.stopPropagation()} style={{background:'#12121e',border:'1px solid rgba(255,170,0,0.25)',borderRadius:20,width:'100%',maxWidth:460,overflow:'hidden'}}>
        <div style={{background:'linear-gradient(135deg,rgba(255,170,0,0.1),rgba(255,107,0,0.06))',borderBottom:'1px solid rgba(255,170,0,0.15)',padding:'16px 20px',display:'flex',alignItems:'center',justifyContent:'space-between'}}><div style={{fontSize:17,fontWeight:800,color:'#fff',fontFamily:'Syne,sans-serif'}}>✏️ Modifier mon profil</div><button onClick={onClose} style={{width:28,height:28,borderRadius:'50%',background:'rgba(255,255,255,0.06)',border:'1px solid rgba(255,255,255,0.1)',color:'rgba(255,255,255,0.5)',fontSize:14,cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center'}}>✕</button></div>
        {done?(<div style={{padding:40,textAlign:'center'}}><div style={{fontSize:44,marginBottom:10}}>✅</div><p style={{color:'#4ade80',fontWeight:700}}>Profil mis à jour !</p></div>):(
          <div style={{padding:'20px',display:'flex',flexDirection:'column',gap:14}}>
            ...
            <div><label style={lbl}>Bio (facultatif)</label><textarea style={{...inp,resize:'vertical',minHeight:70}} placeholder="Quelques mots sur toi…" value={form.bio} onChange={e=>setForm(f=>({...f,bio:e.target.value}))} maxLength={200}/><div style={{textAlign:'right',fontSize:10,color:'rgba(255,255,255,0.3)',marginTop:3}}>{form.bio.length}/200</div></div>
            {err&&<div style={{background:'rgba(248,113,113,0.1)',border:'1px solid rgba(248,113,113,0.25)',borderRadius:10,padding:'10px 14px',fontSize:13,color:'#f87171'}}>⚠️ {err}</div>}
...
function MesVideosSection({videos,loading,contests,onRefresh,router}:{videos:UserVideo[];loading:boolean;contests:Contest[];onRefresh:()=>void;router:any}) {
  ...
  return (
    <div>
      <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:14}}><div style={{fontSize:13,color:'rgba(255,255,255,0.4)'}}>{videos.length} vidéo{videos.length!==1?'s':''}</div><button onClick={()=>router.push('/submit')} style={btnPrimary}>🎬 + Ajouter une vidéo</button></div>
      {loading?(<div style={{textAlign:'center',padding:'40px 20px',color:'rgba(255,255,255,0.3)',fontSize:13}}>⏳ Chargement…</div>):videos.length===0?(
        <div style={{...card,textAlign:'center',padding:'40px 20px'}}><div style={{fontSize:40,marginBottom:12}}>🎬</div><p style={{color:'rgba(255,255,255,0.5)',fontSize:14,marginBottom:16}}>Tu n'as pas encore ajouté de vidéo.</p><button onClick={()=>router.push('/submit')} style={btnPrimary}>Ajouter ma première vidéo</button></div>
      ):(
        <>
          {drafts.length>0&&(<div style={{marginBottom:8}}><div style={{fontSize:11,fontWeight:700,color:'rgba(255,255,255,0.3)',letterSpacing:'.08em',marginBottom:8,textTransform:'uppercase'}}>📝 Brouillons — à soumettre pour validation</div>{drafts.map(v=>(<div key={v.id} style={{...card,border:'1px solid rgba(255,255,255,0.12)'}}><div style={{display:'flex',alignItems:'flex-start',gap:12}}><div style={{width:46,height:46,borderRadius:10,background:'rgba(255,255,255,0.06)',display:'flex',alignItems:'center',justifyContent:'center',fontSize:20,flexShrink:0}}>{DISC_EMOJI[v.discipline??'']??'🎬'}</div><div style={{flex:1,minWidth:0}}><div style={{display:'flex',alignItems:'center',gap:8,marginBottom:4,flexWrap:'wrap'}}><span style={{fontSize:14,fontWeight:700,color:'#fff',overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{v.title}</span><StatusBadge status="draft"/></div><div style={{fontSize:11,color:'rgba(255,255,255,0.4)',marginBottom:10}}>{DISC_FR[v.discipline??'']??v.discipline} · Non soumise à la modération</div><button onClick={()=>setSubId(v.id)} style={{background:'linear-gradient(135deg,#FFAA00,#FF6B00)',border:'none',borderRadius:50,padding:'7px 16px',fontSize:12,fontWeight:700,color:'#000',cursor:'pointer',fontFamily:'DM Sans,sans-serif'}}>🏆 Soumettre pour validation</button></div></div></div>))}</div>)}
          {others.map(v=>(
...
function CompetitionsSection({contests,balance,approvedVideos,onVoted,favContests,favCandidates,onToggleFavContest,onToggleFavCandidate}:{contests:Contest[];balance:number;approvedVideos:UserVideo[];onVoted:(nb:number)=>void;favContests:string[];favCandidates:string[];onToggleFavContest:(id:string)=>void;onToggleFavCandidate:(id:string)=>void}) {
  ...
  return (
    <div>
      <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:14,flexWrap:'wrap',gap:8}}><div style={{fontSize:13,color:'rgba(255,255,255,0.4)'}}>{filtered.length} compétition{filtered.length!==1?'s':''}</div><button onClick={()=>setShowIns(true)} style={{...btnPrimary,padding:'8px 16px',fontSize:12}}>🏆 Inscrire ma vidéo dans une compétition</button></div>
      ...
      {filtered.length===0?(<div style={{textAlign:'center',padding:'40px 20px',color:'rgba(255,255,255,0.3)',fontSize:13}}>{contests.length===0?'Aucune compétition active pour le moment.':'Aucune compétition dans cette catégorie.'}</div>):filtered.map(c=><ContestCard key={c.id} contest={c} userBalance={balance} onVoted={onVoted} isFavContest={favContests.includes(c.id)} favCandidates={favCandidates} onToggleFavContest={()=>onToggleFavContest(c.id)} onToggleFavCandidate={onToggleFavCandidate}/>)}
  ...
  return (
    <div>
      <div style={{background:'linear-gradient(135deg,rgba(255,170,0,0.08),rgba(255,107,0,0.04))',border:'1px solid rgba(255,170,0,0.2)',borderRadius:18,padding:'20px',marginBottom:14,textAlign:'center'}}>
        ...
        <div style={{fontSize:13,color:'rgba(255,255,255,0.4)',marginBottom:16,lineHeight:1.6}}>Explorez 21 matières.<br/>Créez et partagez du contenu éducatif.</div>
...
function FinancesSection({balance,totalEarned,router}:{balance:number;totalEarned:number;router:any}) {
  return (
    <div>
      ...
      <div style={{...card,marginBottom:12}}><div style={{display:'flex',alignItems:'center',justifyContent:'space-between'}}><div><div style={{fontSize:13,fontWeight:600,color:'#fff',marginBottom:3}}>💰 Total encaissé</div><div style={{fontSize:11,color:'rgba(255,255,255,0.4)'}}>Votes reçus sur tes vidéos · Visible par tous</div></div><div style={{textAlign:'right'}}><div style={{fontSize:20,fontWeight:800,color:'#FFAA00',fontFamily:'Syne,sans-serif'}}>{totalEarned.toLocaleString('fr-FR')} F</div><span style={{background:'rgba(255,170,0,0.12)',border:'1px solid rgba(255,170,0,0.25)',borderRadius:20,padding:'2px 8px',fontSize:10,color:'#FFAA00',fontWeight:700}}>PUBLIC</span></div></div></div>
      <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:12}}>
        <div style={{...card,textAlign:'center'}}><div style={{fontSize:28,marginBottom:10}}>⚡</div><div style={{fontSize:14,fontWeight:700,color:'#fff',marginBottom:6}}>Recharger</div><div style={{fontSize:11,color:'rgba(255,255,255,0.4)',marginBottom:14,lineHeight:1.5}}>Acheter des étoiles et cœurs via FedaPay, MTN, Moov.</div><button onClick={()=>router.push('/recharge')} style={btnPrimary}>Recharger mon compte</button></div>
        <div style={{...card,textAlign:'center'}}><div style={{fontSize:28,marginBottom:10}}>💸</div><div style={{fontSize:14,fontWeight:700,color:'#fff',marginBottom:6}}>Retrait</div><div style={{fontSize:11,color:'rgba(255,255,255,0.4)',marginBottom:14,lineHeight:1.5}}>Retirer tes gains vers ton Mobile Money ou compte bancaire.</div><button onClick={()=>router.push('/retrait')} style={{...btnSecondary,display:'inline-block'}}>Faire un retrait</button></div>
      </div>
    </div>
  );
}
  ...
  return (
    <div style={{minHeight:'100vh',background:'#0a0a0f',color:'#f0f0f0',fontFamily:'DM Sans,sans-serif',paddingBottom:60}}>
      ...
      <div style={{maxWidth:660,margin:'0 auto',padding:'20px 16px'}}>
        ...
        {activeTab==='competitions'&&(loading?<div style={{textAlign:'center',padding:'40px',color:'rgba(255,255,255,0.3)'}}>⏳ Chargement…</div>:<CompetitionsSection contests={contests} balance={balance} approvedVideos={approved} onVoted={nb=>setBalance(nb)} favContests={favContests} favCandidates={favCandidates} onToggleFavContest={toggleFavContest} onToggleFavCandidate={toggleFavCandidate}/>)}
