...
export async function uploadVideo(params: UploadVideoParams) {
  ...
  const { data: video, error: dbErr } = await supabase.from('videos').insert({ voter_id: userId, discipline, track_title: trackTitle, track_artist: trackArtist, track_genre: trackGenre, title, description, storage_path: storagePath, storage_url: signedUrl?.signedUrl, file_size_mb: fileSizeMb, format: ext, status: 'pending' }).select().single();
...
export async function moderateVideo(videoId: string, moderatorId: string, decision: 'approved' | 'rejected', reason?: string) {
  ...
  await supabase.from('notifications').insert({ voter_id: video.voter_id, type: `video_${decision}`, title: decision === 'approved' ? 'Video validee' : 'Video refusee', message: decision === 'approved' ? 'Votre video a ete validee' : `Refusee: ${reason}`, data: { video_id: videoId, decision, reason } });
  return video;
}
...
export async function getUserVideos(userId: string) {
  const { data, error } = await supabase.from('videos').select('*').eq('voter_id', userId).order('created_at', { ascending: false });
  if (error) throw error;
  return data || [];
}
...
export async function deleteVideo(videoId: string, userId: string) {
  const { data: video } = await supabase.from('videos').select('storage_path, voter_id').eq('id', videoId).single();
  if (!video) throw new Error('VIDEO_NOT_FOUND');
  if (video.voter_id !== userId) throw new Error('FORBIDDEN');
...
async function notifyModerators(videoId: string, userId: string, discipline: Discipline) {
  ...
  await supabase.from('notifications').insert(mods.map(mod => ({ voter_id: mod.id, type: 'video_pending', title: 'Nouvelle video', message: `Video de ${discipline} en attente`, data: { video_id: videoId, submitter_id: userId, discipline } })));
}
...
export async function addComment(videoId: string, userId: string, content: string) {
  ...
  const { data: video } = await supabase.from('videos').select('id, voter_id').eq('id', videoId).single();
  if (!video) throw new Error('VIDEO_NOT_FOUND');
  const { data: comment, error } = await supabase.from('comments').insert({ video_id: videoId, voter_id: userId, content: content.trim() }).select('*').single();
  if (error) throw error;
  if (video.voter_id !== userId) { await supabase.from('notifications').insert({ voter_id: video.voter_id, type: 'new_comment', title: 'Nouveau commentaire', message: 'Quelquun a commente votre video', data: { video_id: videoId, comment_id: comment.id } }); }
  return comment;
}
